...
pub fn exec_run(args: &[Value16]) -> HudHudResult<Value16> {
    ...
    let output = cmd.output().map_err(|e| {
        runtime_error(format!(
            "exec.run error: program '{}' not found or failed to start: {}",
            program, e
        ))
    })?;
...
pub fn exec_output(args: &[Value16]) -> HudHudResult<Value16> {
    ...
    let output = cmd.output().map_err(|e| {
        runtime_error(format!(
            "exec.output error: program '{}' not found or failed to start: {}",
            program, e
        ))
    })?;
...
pub fn exec_lines(args: &[Value16]) -> HudHudResult<Value16> {
    ...
    let output = cmd.output().map_err(|e| {
        runtime_error(format!(
            "exec.lines error: program '{}' not found or failed to start: {}",
            program, e
        ))
    })?;
