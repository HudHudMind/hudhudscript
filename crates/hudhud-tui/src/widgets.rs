...
fn parse_style(cfg: &hudhudscript_bytecode::ObjMap) -> Style {
    ...
    if let Some(fg) = cfg.get("fg").and_then(|v| v.as_string()) {
        style = style.fg(parse_color(&fg));
    }
    if let Some(bg) = cfg.get("bg").and_then(|v| v.as_string()) {
        style = style.bg(parse_color(&bg));
    }
    if cfg.get("bold").and_then(|v| v.as_bool()).unwrap_or(false) {
        style = style.add_modifier(Modifier::BOLD);
    }
    if cfg.get("italic").and_then(|v| v.as_bool()).unwrap_or(false) {
...
pub fn tui_paragraph(args: &[Value16]) -> CompileResult<Value16> {
    ...
    COMMAND_QUEUE.with(|q| {
        q.borrow_mut()
            .push(RenderCommand::Paragraph(text, style, area))
    });
    Ok(Value16::null())
}
...
pub fn tui_block(args: &[Value16]) -> CompileResult<Value16> {
    ...
    let area = cfg
        ...
        .unwrap_or_default();
    COMMAND_QUEUE.with(|q| {
        q.borrow_mut().push(RenderCommand::Block(
            title,
            borders,
            border_style,
            style,
...
pub fn tui_gauge(args: &[Value16]) -> CompileResult<Value16> {
    ...
    COMMAND_QUEUE.with(|q| {
        q.borrow_mut()
            .push(RenderCommand::Gauge(ratio, label, style, area))
    });
    Ok(Value16::null())
}
    ...
    for cmd in drain_queue() {
        match cmd {
            ...
            RenderCommand::Gauge(ratio, label, style, area) => {
                frame.render_widget(
                    Gauge::default().ratio(ratio).label(label).style(style),
                    area,
                );
            }
        }
