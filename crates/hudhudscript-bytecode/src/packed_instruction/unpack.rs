...
pub fn unpack(packed: u32) -> Option<Instruction> {
    ...
    match opcode {
        ...
        OP_JUMP_IF_FALSE => Some(Instruction::JumpIfFalse { src: 255, offset: arg2 as i16 }),
        OP_JUMP_IF_TRUE => Some(Instruction::JumpIfTrue { src: 255, offset: arg2 as i16 }),
        OP_JUMP_IF_FALSE_R => Some(Instruction::JumpIfFalse { src: arg1, offset: arg2 as i16 }),
        OP_JUMP_IF_TRUE_R => Some(Instruction::JumpIfTrue { src: arg1, offset: arg2 as i16 }),
        ...
        OP_FOR_IN => Some(Instruction::ForIn { iter_reg: 255, var_sym_idx: arg2 as u16, end_offset: 0 }),
        OP_RECEIVE => Some(Instruction::Receive { var_sym_idx: arg2 as u16, src: 255 }),

        // arg2 = i16 relative offset
        OP_ITER_NEXT => Some(Instruction::IterNext { iter_reg: 255, var_sym_idx: 0, end_offset: arg2 as i16 }),
        ...
        // call-family
        OP_METHOD_CALL => Some(Instruction::MethodCall { dst: 255, obj: 255, payload_idx: arg2 as u16, first_arg: 0, arg_count: 0 }),
        OP_NEW_INSTANCE => None, // Back compat: register-based NewInstance
        OP_SPAWN => None, // Back compat: use Spawn with register operands
        OP_SUPER_CALL => Some(Instruction::SuperCall { dst: 255, payload_idx: arg2 as u16, first_arg: 0, arg_count: 0 }),
        ...
        // IndexFast removed — use Index (not packable in u32)
        OP_INT_INCR_SLOT => Some(Instruction::IntIncrSlot { slot: arg2 as u32 }),

        // Register-based VM opcodes
        OP_INT_EQ_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::IntCmp { dst: arg1, src1, src2, op: 4 })
        }
        OP_INT_LT_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::IntCmp { dst: arg1, src1, src2, op: 0 })
        }
        OP_INT_LE_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::IntCmp { dst: arg1, src1, src2, op: 1 })
        }
        OP_INT_NE_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::IntCmp { dst: arg1, src1, src2, op: 5 })
        }
        OP_MOVE_RR => Some(Instruction::Move { dst: arg1, src: arg2 as u8 }),
        OP_INT_ADD_RI => {
            let src = (arg2 & 0xFF) as u8;
            let imm = ((arg2 >> 8) as i8) as i16;
            Some(Instruction::IntAddI { dst: arg1, src, imm })
        }
        OP_INT_SUB_RI => {
            let src = (arg2 & 0xFF) as u8;
            let imm = ((arg2 >> 8) as i8) as i16;
            Some(Instruction::IntSubI { dst: arg1, src, imm })
        }
        OP_INT_MUL_RI => {
            let dst = arg1;
            let src = (arg2 & 0xFF) as u8;
        ...
        OP_NEG_R => Some(Instruction::Neg { dst: arg1, src: arg2 as u8 }),
        OP_NOT_R => Some(Instruction::Not { dst: arg1, src: arg2 as u8 }),
        OP_ARRAY_PUSH_RRR => {
            let arr = ((arg2 >> 8) & 0xFF) as u8;
            let val = (arg2 & 0xFF) as u8;
            Some(Instruction::ArrayPush { dst: arg1, arr, val })
        }
        OP_INDEX_RRR => {
            let obj = ((arg2 >> 8) & 0xFF) as u8;
            let idx = (arg2 & 0xFF) as u8;
            Some(Instruction::Index { dst: arg1, obj, idx })
        }
        OP_STRCAT_RRR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::StrCat { dst: arg1, src1, src2 })
        }
        OP_STRCAT_MUT_RR => {
            Some(Instruction::StrCatMut { dst: arg1, src2: arg2 as u8 })
        }
        OP_STRING_INDEX_OF_RRR => {
            let haystack = ((arg2 >> 8) & 0xFF) as u8;
            let needle = (arg2 & 0xFF) as u8;
            Some(Instruction::StringIndexOf { dst: arg1, haystack, needle })
        }
        OP_STRING_CONTAINS_RRR => {
            let haystack = ((arg2 >> 8) & 0xFF) as u8;
            let needle = (arg2 & 0xFF) as u8;
            Some(Instruction::StringContains { dst: arg1, haystack, needle })
        }
        OP_INT_ADD_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::IntAdd { dst: arg1, src1, src2 })
        }
        OP_INT_SUB_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::IntSub { dst: arg1, src1, src2 })
        }
        OP_INT_MUL_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::IntMul { dst: arg1, src1, src2 })
        }
        OP_INT_MOD_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::IntMod { dst: arg1, src1, src2 })
        }
        OP_LOAD_INT_CONST_R => Some(Instruction::LoadIntConst { dst: arg1, const_idx: arg2 }),
        ...
        OP_NUM_ADD_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::NumAdd { dst: arg1, src1, src2 })
        }
        OP_NUM_SUB_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::NumSub { dst: arg1, src1, src2 })
        }
        OP_NUM_MUL_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::NumMul { dst: arg1, src1, src2 })
        }
        OP_NUM_DIV_RR => {
            let src1 = ((arg2 >> 8) & 0xFF) as u8;
            let src2 = (arg2 & 0xFF) as u8;
            Some(Instruction::NumDiv { dst: arg1, src1, src2 })
        }
        // IndexAssign (133)
        OP_INDEX_ASSIGN_RRR => {
            let idx = ((arg2 >> 8) & 0xFF) as u8;
            let val = (arg2 & 0xFF) as u8;
            Some(Instruction::IndexAssign { obj: arg1, idx, val })
        }
        // Float immediate arithmetic (134-137)
        OP_NUM_ADD_RI => {
            let src = (arg2 & 0xFF) as u8;
            let imm = ((arg2 >> 8) as i8) as i16;
            Some(Instruction::NumAddI { dst: arg1, src, imm })
        }
        OP_NUM_SUB_RI => {
            let src = (arg2 & 0xFF) as u8;
            let imm = ((arg2 >> 8) as i8) as i16;
            Some(Instruction::NumSubI { dst: arg1, src, imm })
        }
        OP_NUM_MUL_RI => {
            let src = (arg2 & 0xFF) as u8;
            let imm = ((arg2 >> 8) as i8) as i16;
            Some(Instruction::NumMulI { dst: arg1, src, imm })
        }
        OP_NUM_DIV_RI => {
            let src = (arg2 & 0xFF) as u8;
            let imm = ((arg2 >> 8) as i8) as i16;
            Some(Instruction::NumDivI { dst: arg1, src, imm })
        }
