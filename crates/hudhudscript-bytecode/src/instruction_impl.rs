...
impl Instruction {
    ...
    pub fn max_register(&self) -> u8 {
        ...
        let mr = match *self {
            ...
            FLoadNum { src, .. } => src,
            FStoreNum { dst, .. } => dst,
            FAdd { .. } | FSub { .. } | FMul { .. } | FDiv { .. } | FSin { .. }
            | FCos { .. } | FSqrt { .. } | FConst { .. } | FMove { .. } => 0,
        };
...
impl Instruction {
    ...
    pub fn register_effects(&self) -> RegisterEffects {
        ...
        fn rw(reads: &[u8], writes: Option<u8>) -> RegisterEffects {
            ...
            RegisterEffects { reads: r, read_range: None, writes, barrier: false }
        }
        fn barrier() -> RegisterEffects {
            RegisterEffects { barrier: true, ..Default::default() }
        }
        match *self {
            // ── yükler (yalnız yazar) ─────────────────────────────
            LoadConst { dst, .. } | LoadNumConst { dst, .. } | LoadIntConst { dst, .. }
            | LoadGlobal { dst, .. } | LoadClosureSlot { dst, .. } => rw(&[], Some(dst)),
            // ── saklamalar (yalnız okur; hedef register değil) ────
            StoreGlobal { src, .. } | DeclGlobal { src, .. } | DeclStore { src, .. }
            | StoreConst { src, .. } | StoreClosureSlot { src, .. } => rw(&[src], None),
            StoreGlobalConst { .. } => rw(&[], None),
            // ── veri akışı ────────────────────────────────────────
            Move { dst, src } | Neg { dst, src } | Not { dst, src } | NumSqrt { dst, src }
            | NumSin { dst, src } | NumCos { dst, src } => {
                rw(&[src], Some(dst))
            }
            ...
            FAdd { .. } | FSub { .. } | FMul { .. } | FDiv { .. } | FSin { .. }
            | FCos { .. } | FSqrt { .. } | FConst { .. } | FMove { .. } => rw(&[], None),
            // ── aritmetik RR ──────────────────────────────────────
            IntAdd { dst, src1, src2 } | IntSub { dst, src1, src2 } | IntMul { dst, src1, src2 }
            | IntDiv { dst, src1, src2 } | IntMod { dst, src1, src2 }
            | NumAdd { dst, src1, src2 } | NumSub { dst, src1, src2 }
            | NumMul { dst, src1, src2 } | NumDiv { dst, src1, src2 }
            | NumMod { dst, src1, src2 } | FloatAdd { dst, src1, src2 }
            | FloatMul { dst, src1, src2 } | IntCmp { dst, src1, src2, .. }
            | StrCat { dst, src1, src2 } => rw(&[src1, src2], Some(dst)),
            // ── aritmetik imm ─────────────────────────────────────
            IntAddI { dst, src, .. } | IntSubI { dst, src, .. } | IntMulI { dst, src, .. }
            | IntDivI { dst, src, .. } | IntModI { dst, src, .. } | IntCmpI { dst, src, .. }
            | IntModCmpI { dst, src, .. } | NumAddI { dst, src, .. } | NumSubI { dst, src, .. }
            | NumMulI { dst, src, .. } | NumDivI { dst, src, .. } => rw(&[src], Some(dst)),
            ...
            FloatMulAdd { dst, mul1, mul2, add } => rw(&[mul1, mul2, add], Some(dst)),
            IntMulMod { dst, src1, src2, src3 } => rw(&[src1, src2, src3], Some(dst)),
            IntMulModI { dst, src1, src2, .. } => rw(&[src1, src2], Some(dst)),
            PropertySubAssign { obj, src, .. } => rw(&[obj, src], None),
            StrCatMut { dst, src2 } => rw(&[dst, src2], None),
            StrCat3 { dst, a, b, c } => rw(&[a, b, c], Some(dst)),
            StrCharEqRR { dst, src_s, src_i, src_j } => rw(&[src_s, src_i, src_j], Some(dst)),
            StringIndexOf { dst, haystack, needle } | StringContains { dst, haystack, needle } => {
                rw(&[haystack, needle], Some(dst))
            }
            StringConcat { regs_start, count, dst } => RegisterEffects {
                reads: [None; 4],
            ...
            // ── koleksiyon ────────────────────────────────────────
            Index { dst, obj, idx } | IndexArray { dst, obj, idx }
            | IndexStringAscii { dst, obj, idx } => rw(&[obj, idx], Some(dst)),
            IndexAssign { obj, idx, val } | IndexAssignArray { obj, idx, val } => {
                rw(&[obj, idx, val], None)
            }
            Index2D { dst, obj, idx1, idx2 } => rw(&[obj, idx1, idx2], Some(dst)),
            IndexAssign2D { obj, idx1, idx2, val } => rw(&[obj, idx1, idx2, val], None),
            ...
            MakeArray { dst, count } => {
                if count == 0 { rw(&[], Some(dst)) } else { barrier() }
            }
            ...
            | FinallyEnd | FinallyExit(..) | LoopBegin(..) | LoopEnd => rw(&[], None),
            JumpIfFalse { src, .. } | JumpIfTrue { src, .. } => rw(&[src], None),
            IntCmpIJumpIfFalse { src, .. } | IntCmpIJumpIfTrue { src, .. } => rw(&[src], None),
            IntCmpRRJumpIfFalse { src1, src2, .. } | IntLtRRJumpIfFalse { src1, src2, .. }
            ...
            IntAddReturn { src1, src2 } | IntSubReturn { src1, src2 }
            | IntMulReturn { src1, src2 } | IntDivReturn { src1, src2 } => {
                rw(&[src1, src2], Some(255))
            }
            IntCmpIReturn { src, .. } => rw(&[src], Some(255)),
            // ── çağrılar ──────────────────────────────────────────
            Call { dst, first_arg, arg_count, .. } => RegisterEffects {
                reads: [None; 4],
                read_range: Some((first_arg, arg_count)),
                writes: Some(dst),
                barrier: false,
            },
            MethodCall { dst, obj, first_arg, arg_count, .. } => RegisterEffects {
                reads: [Some(obj), None, None, None],
                read_range: Some((first_arg, arg_count)),
                writes: Some(dst),
                barrier: false,
            },
            SuperCall { dst, first_arg, arg_count, .. } => RegisterEffects {
                reads: [None; 4],
                read_range: Some((first_arg, arg_count)),
                writes: Some(dst),
                barrier: false,
            },
            TailCall { func_reg, first_arg_reg, arg_count } => RegisterEffects {
            ...
            CallSpread(..) | MethodCallSpread(..) | IntSubCall1(..) | IntAddCall1(..)
            | IntLeJumpIfFalse(..) | IntLtJumpIfFalse(..) | IntLtRRJumpPacked(..)
            | IntLeRRJumpPacked(..) | IntCmpRRJumpPacked { .. } | CharDispatch { .. }
            | ForIn { .. } | IterNext { .. }
            | EnumDecl(..) | MatchVariant(..) | BindVar(..) | ClassDecl(..) | TraitCheck(..)
            | LoadModule(..) | DefineFunction(..) | GetStatic(..) | ClassStaticDecl(..)
            | DestructArray(..) | DestructObject(..) | NewInstance { .. }
            | MakeGenerator { .. } | Spawn { .. } | Despawn { .. } | ViewAs { .. }
            | Send { .. } | Receive { .. } | Require { .. } | Perform { .. } | Yield { .. }
            | Await { .. } | Remember { .. } | Recall { .. } | Forget { .. } => barrier(),
        }
    }
    ...
    pub fn branch_target(&self, ip: usize) -> Option<usize> {
        ...
        let rel: i64 = match *self {
            ...
            IntAddIJump { offset, .. } | IntSubIJump { offset, .. }
