...
fn pack_index_array_returns_some() {
    let instr = Instruction::IndexArray {
        dst: 5,
        obj: 10,
        idx: 3,
    };
...
fn pack_index_string_ascii_returns_some() {
    let instr = Instruction::IndexStringAscii {
        dst: 5,
        obj: 10,
        idx: 3,
    };
...
fn unpack_pack_index_array_roundtrip() {
    let original = Instruction::IndexArray {
        dst: 5,
        obj: 10,
        idx: 3,
    };
...
fn unpack_pack_index_string_ascii_roundtrip() {
    let original = Instruction::IndexStringAscii {
        dst: 7,
        obj: 12,
        idx: 4,
    };
...
fn packed_opcode_differs_between_index_array_and_string_ascii() {
    ...
    let arr = pack(&Instruction::IndexArray {
        dst: 1,
        obj: 2,
        idx: 3,
    })
    .unwrap();
    let str = pack(&Instruction::IndexStringAscii {
        dst: 1,
        obj: 2,
        idx: 3,
    })
    ...
    assert_ne!(
        arr, str,
        "IndexArray and IndexStringAscii packed values must differ"
    );
}

#[test]
fn generic_index_still_packs() {
    let p = pack(&Instruction::Index {
        dst: 1,
        obj: 2,
        idx: 3,
    });
