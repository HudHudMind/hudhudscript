...
pub fn log_init(config: &Value16) -> CompileResult<Value16> {
    ...
    match (format.as_str(), output.as_str()) {
        ("json", "stdout") => registry
            .with(fmt::layer().json().with_writer(std::io::stdout))
            .init(),
        ("json", "stderr") => registry
            .with(fmt::layer().json().with_writer(std::io::stderr))
            .init(),
        ("pretty", "stdout") => registry
            .with(fmt::layer().pretty().with_writer(std::io::stdout))
            .init(),
        ("pretty", "stderr") => registry
            .with(fmt::layer().pretty().with_writer(std::io::stderr))
            .init(),
        ("compact", "stdout") => registry
            .with(fmt::layer().compact().with_writer(std::io::stdout))
            .init(),
        ("compact", "stderr") => registry
...
fn fields_to_string(fields: Option<&Value16>) -> String {
    let Some(obj) = fields.and_then(|v| v.as_object()) else {
        return String::new();
    };
