...
pub fn serve(args: &[Value16]) -> HudHudResult<Value16> {
    let opts = args
        ...
        .ok_or_else(|| type_error("object", "", "Web.serve"))?;
    ...
    let socket = if reuse_port {
        ...
        let sock = Socket::new(domain, Type::STREAM, Some(Protocol::TCP))
            .map_err(|e| runtime_error(format!("Web.serve: socket: {}", e)))?;
        sock.set_reuse_address(true)
            .map_err(|e| runtime_error(format!("Web.serve: reuse_address: {}", e)))?;
        // SO_REUSEPORT is Unix-only; Windows uses SO_REUSEADDR above.
        #[cfg(unix)]
        sock.set_reuse_port(true)
            .map_err(|e| runtime_error(format!("Web.serve: reuse_port: {}", e)))?;
        sock.set_nonblocking(false)
            .map_err(|e| runtime_error(format!("Web.serve: nonblocking: {}", e)))?;
        sock.bind(&addr.into())
            .map_err(|e| runtime_error(format!("Web.serve: bind {}: {}", addr, e)))?;
        sock.listen(1024)
            .map_err(|e| runtime_error(format!("Web.serve: listen: {}", e)))?;
