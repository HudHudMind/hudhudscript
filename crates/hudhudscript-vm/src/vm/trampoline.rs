...
use hudhudscript_bytecode::{Bytecode, FunctionChunk, Value16, SymId};
...
impl VM {
    pub(crate) fn exec_call_push_frame(
        ...
        let reg_size = if first_arg > 0 { first_arg as usize } else { callee_reg_count };
        self.registers.advance(reg_size);
        // PERF-T2-1: Compiler invariant — every register is written before it
        // is read (param_slots bind params, DeclLocal/StoreLocal init locals).
        // zero_frame was unconditional memset = 1.15% fib, 35% ack.  Removed.
        ...
        // written by StoreLocal during execute_chunk. Creating a cell here for
        ...
        for cap_name in chunk.captures.iter() {
            ...
            let is_param = params.iter().any(|p| {
                p == cap_name || p.strip_prefix("...").map_or(false, |s| s == cap_name)
            });
            if !is_param {
                continue;
            }
                    ...
                    if let Ok(idx) = local_syms.binary_search_by_key(&sym_id, |(s, _, _)| *s) {
                        ...
                        if slot >= 0 {
                            ...
                            if local_idx < self.registers.len() {
                                ...
                                if !self.scope_cells.last().map_or(false, |c| c.contains_key(cap_name)) {
    ...
    pub(crate) fn run_frame_loop(&mut self, bytecode: &Bytecode, main_packed: &[u32], stop_depth: usize) -> CompileResult<bool> {
        ...
        let result = 'outer: loop {
            ...
            let (instructions, constants, packed_slice, chunk_sp, mut ip, dst) = {
                ...
                if frame.chunk_ptr.is_null() {
                    (&bytecode.instructions, &bytecode.constants, main_packed, None, frame.ip, frame.dst)
                } else {
                    ...
                    (&chunk.instructions, &chunk.constants, packed_slice, chunk_sp, frame.ip, frame.dst)
                }
            };
            match self.execute_instructions(instructions, constants, bytecode, packed_slice, chunk_sp, &mut ip) {
                Ok(hit_return) => {
                    self.frame_stack[frame_idx].ip = ip;
                    if let Some((func_sym, arg_count, first_arg, dst)) = self.pending_call.take() {
                        ...
                        // Eliminates one Rust stack frame per cached call (~7% Ir).
                        ...
                        let cache_hit = if let Some(cached) = self.call_cache.get(sym_id)
                            .and_then(|slot| slot.as_ref().map(|(ptr, params_ptr)| (*ptr, *params_ptr)))
                        {
                            let chunk_ref = unsafe { &*cached.0 };
                            if !chunk_ref.is_async {
                                let params = unsafe { &*cached.1 };
                                if chunk_ref.captures.is_empty() && !chunk_ref.is_generator {
                                    self.fast_call_push_frame(chunk_ref, params, args, func_sym, first_arg, dst)?;
                                } else {
                                    self.exec_call_push_frame(chunk_ref, params, args, bytecode, func_sym, None, first_arg, dst)?;
                                }
                        ...
                        if !cache_hit {
                            self.exec_call(func_sym, arg_count, first_arg, dst, bytecode, 0)?;
                        }
                        if is_super {
