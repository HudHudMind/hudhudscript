...
pub fn dispatch(method: StdinMethodId, args: &[Value16]) -> HudHudResult<Value16> {
    match method {
        ...
        StdinMethodId::Line => match read_line_eof()? {
            None => Ok(Value16::null()),
            Some(s) => Ok(Value16::string(s)),
        },
            ...
            match rpassword::read_password() {
                ...
                Err(e) => {
                    ...
                    Err(runtime_error(format!(
                        "stdin.password: no TTY available: {}",
                        e
                    )))
                }
            }
        ...
        StdinMethodId::Number => {
            ...
            match read_line_eof()? {
                None => Ok(Value16::null()),
                Some(s) => s.trim().parse::<f64>().map(Value16::number).map_err(|_| {
                    runtime_error(format!("stdin.number: '{}' is not a number", s.trim()))
                }),
            }
        }
        StdinMethodId::Int => {
            ...
            match read_line_eof()? {
                None => Ok(Value16::null()),
                Some(s) => s.trim().parse::<i64>().map(Value16::int).map_err(|_| {
                    runtime_error(format!("stdin.int: '{}' is not an integer", s.trim()))
                }),
            }
        }
        StdinMethodId::All => {
        ...
        StdinMethodId::Lines => {
            use std::io::BufRead;
            let lines: Vec<Value16> = std::io::stdin()
                .lock()
                .lines()
                .map_while(Result::ok)
                .map(Value16::string)
