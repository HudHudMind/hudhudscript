...
/// Identifier-start predicate with ASCII fast-path.
///
/// 99% of real source characters are ASCII.  `char::is_alphabetic` goes
/// through a multi-level Unicode property table which is correct for
/// multilingual identifiers (Arabic متغير, Japanese 間, Cyrillic and
/// so on) but ~10× slower than a direct ASCII range check.  Dispatch
/// on `(ch as u32) < 128` first, fall back to the full Unicode check
/// for non-Latin scripts — no multilingual feature lost.
///
/// Audit v3 Finding 18.1 — Aho-Lam-Sethi-Ullman Dragon Book §3.7 DFA
/// character classification fast-path.
#[inline]
...
fn japanese_numeral_char_to_number(ch: char) -> Option<f64> {
    match ch {
        '〇' | '零' => Some(0.0),
        '一' => Some(1.0),
        '二' => Some(2.0),
