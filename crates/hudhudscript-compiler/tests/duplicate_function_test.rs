...
fn test_duplicate_toplevel_function_errors() {
    ...
    assert!(err.contains("line 1"), "should mention original definition line 1, got: {}", err);
}
...
fn test_error_message_not_unsupported_feature() {
    ...
    assert!(!err.contains("unsupported feature"), "should not say unsupported: {}", err);
    assert!(err.contains("Duplicate") || err.contains("already defined"), "got: {}", err);
}
