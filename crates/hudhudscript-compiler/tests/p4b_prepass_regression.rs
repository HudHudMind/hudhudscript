...
fn array_then_unknown_degraded_to_generic() {
    ...
    let insns =
        compile_instructions("fn f(x) { return x[0]; } print(f([1,2])); print(f(unknown_var));");
    assert!(
...
fn unknown_then_array_degraded_to_generic() {
    ...
    let insns =
        compile_instructions("fn f(x) { return x[0]; } print(f(unknown_var)); print(f([1,2]));");
    assert!(
        has_instruction(&insns, |i| matches!(i, Instruction::Index { .. })),
...
fn function_declared_before_call_source_order_preserved() {
    ...
    let insns = compile_instructions("fn add1(x) { return x + 1; } print(add1(41));");
    // The function must exist in bytecode (declaration compiled before call)
    let has_fn = insns
        .iter()
        .any(|i| matches!(i, Instruction::IntAddI { .. }));
...
fn p3b_top_level_inline_preserved() {
    ...
    assert!(
        call_count <= 2,
        "caller must have inlined add1, got {} calls",
        call_count
    );
}
...
fn scope_aware_let_array_f_call_yields_indexarray() {
    let insns = compile_instructions("fn f(x) { return x[0]; } let a = [1,2]; print(f(a));");
    assert!(
...
fn multi_param_both_typed() {
    ...
    let insns = compile_instructions("fn f(a, b) { return a[0]; } print(f([1,2], \"hi\"));");
    assert!(
        has_instruction(&insns, |i| matches!(i, Instruction::IndexArray { .. })),
...
fn missing_arg_becomes_unknown() {
    ...
    let insns = compile_instructions("fn f(a, b) { return a[0]; } print(f([1,2]));");
    assert!(
        has_instruction(&insns, |i| matches!(i, Instruction::IndexArray { .. })),
...
fn outer_scope_does_not_leak_into_function_param() {
    ...
    let insns = compile_instructions("let x = [1,2]; fn f(x) { return x[0]; } print(f(\"hi\"));");
...
fn array_to_string_reassignment_no_indexarray() {
    ...
    let insns =
        compile_instructions("fn f(a) { return a[0]; } let x = [1,2]; x = \"hi\"; print(f(x));");
    assert!(
        !has_instruction(&insns, |i| matches!(i, Instruction::IndexArray { .. })),
