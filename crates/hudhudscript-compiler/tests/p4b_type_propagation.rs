...
fn function_called_with_array_uses_indexarray() {
    ...
    let has_index_array = chunk.instructions.iter().any(|i| {
        matches!(i, Instruction::IndexArray { .. })
    });
    let has_generic_index = chunk.instructions.iter().any(|i| {
        matches!(i, Instruction::Index { .. })
    });

    eprintln!("f body instructions: {:?}", chunk.instructions);
    assert!(has_index_array, "f(a) called with array should emit IndexArray, got generic={}", has_generic_index);
}
