...
fn extract_char_eq(condition: &Expr) -> Option<(&str, u8)> {
    if let Expr::Binary { op: hudhudscript_ast::BinaryOp::Eq, left, right, .. } = condition {
...
fn collect_char_dispatch<'a>(
    ...
    match else_branch {
        Some(Stmt::If {
            ...
            else_branch,
            ..
        }) => collect_char_dispatch(condition, then_branch, else_branch.as_deref(), branches, default, var_name),
        Some(other) => {
            *default = Some(other);
            true
        }
...
fn compile_char_dispatch<'a, T: CompileTarget>(
    ...
    target.ct_emit(Instruction::CharDispatch { src: src_reg, table_idx });
    ...
    let default_offset = (default_start_ip as i64).wrapping_sub(dispatch_ip as i64).wrapping_sub(1) as i16;
    table.fill(default_offset);
    for (byte, body_start) in branch_start_ips {
        let off = (body_start as i64).wrapping_sub(dispatch_ip as i64).wrapping_sub(1) as i16;
    ...
    match stmt {
        ...
        Stmt::If {
            ...
            // C6: try to compile an if-else-if chain of single-char equality as CharDispatch.
            {
                ...
                if collect_char_dispatch(condition, then_branch, else_branch.as_deref(), &mut branches, &mut default, &mut var_name) {
                    if branches.len() >= 3 {
