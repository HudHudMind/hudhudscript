...
pub(super) fn compile_for_in(
    ...
    let iter_reg =
        crate::compiler::expr::compile_reg::compile_expr_to_reg(target, iterable, &mut regs);
    let var_sym = target.ct_sym(variable);
    target.ct_declare_local(variable, false)?;
    target.ct_emit(Instruction::ForIn {
        iter_reg,
        var_sym_idx: var_sym.0 as u16,
        end_offset: 0,
    });
    let loop_top = target.ct_current_ip();
    target.ct_emit(Instruction::IterNext {
        iter_reg,
        var_sym_idx: var_sym.0 as u16,
        end_offset: 0,
    });
    ...
    target.ct_patch(
        ...
        Instruction::IterNext {
            iter_reg,
            var_sym_idx: var_sym.0 as u16,
            end_offset: jump_off(loop_top, end) as i16,
        },
    );
    Ok(())
}
...
pub(super) fn compile_for_range(
    ...
    target.ct_emit(Instruction::IntLeRRJumpIfFalse {
        src1: iter_load1,
        src2: stop_reg2,
        offset: 0,
    });
    ...
    target.ct_patch(
        ...
        Instruction::JumpIfFalse {
            src: dir_load,
            offset: jump_off(asc_jump, desc_start) as i16,
        },
    );
    ...
    target.ct_patch(
        cond_done_jump,
        Instruction::Jump(jump_off(cond_done_jump, cond_check)),
    );
    ...
    let step_reg = if let Some(step_expr) = step {
        ...
        target.ct_patch(
            ...
            Instruction::JumpIfFalse {
                src: dir_load2,
                offset: jump_off(step_asc_jump, neg_step_start) as i16,
            },
        );
        ...
        target.ct_emit(Instruction::LoadNumConst {
            dst: neg_one_reg,
            const_idx: neg_one_idx as u16,
        });

        let step_done = target.ct_current_ip();
        target.ct_patch(
            step_done_jump,
            Instruction::Jump(jump_off(step_done_jump, step_done)),
        );
