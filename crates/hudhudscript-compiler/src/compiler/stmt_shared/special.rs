...
pub(super) fn compile_stmt_part3(
    ...
    match stmt {
        ...
        Stmt::Spawn {
            ...
            target.ct_emit(Instruction::Spawn {
                name_sym: name_sym.0,
                first_arg,
                arg_count: argc,
            });
        ...
        Stmt::Send {
            ...
            let msg_r = crate::compiler::expr::compile_reg::compile_expr_to_reg(
                target,
                message,
                &mut RegAlloc::new_with_base(target.ct_next_local_reg())?,
            );
            let tgt_r = crate::compiler::expr::compile_reg::compile_expr_to_reg(
                target,
                send_target,
                &mut RegAlloc::new_with_base(target.ct_next_local_reg())?,
            );
            target.ct_emit(Instruction::Send {
                message: msg_r,
                target: tgt_r,
            });
        }
        ...
        Stmt::Receive {
            ...
            target.ct_emit(Instruction::Receive {
        ...
        Stmt::Perform { action, .. } => {
            ...
            let _r = crate::compiler::expr::compile_reg::compile_expr_to_reg(
                target,
                action,
                &mut RegAlloc::new_with_base(target.ct_next_local_reg())?,
            );
        ...
        Stmt::Remember {
            ...
            target.ct_emit(Instruction::Remember {
        ...
        Stmt::Forget {
            ...
            let store_sym = store_name.as_ref().map(|s| target.ct_sym(s));
            let idx = target.ct_add_opt_sym_payload(store_sym);
            target.ct_emit(Instruction::Forget {
                store_idx: idx as u16,
                src: r,
            });
        }
