...
pub(super) fn compile_assignment(
    ...
    if let Expr::Identifier(ref name, _) = assign_target {
        ...
        if let Some(chain) = chain_opt {
            let mut regs =
                crate::compiler::regalloc::RegAlloc::new_with_base(target.ct_next_local_reg())
                    .expect("out of register zones");
            let dst = regs
                .alloc(target.ct_current_ip(), target.ct_current_ip() + 255)
            ...
            for expr in chain {
                let src = crate::compiler::expr::compile_reg::compile_expr_to_reg(
                    target, expr, &mut regs,
                );
            ...
            if let Some((mul_expr, add_expr)) = fma_opt {
                if let Some(local_reg) = target.ct_local_reg(name) {
                    ...
                    let mut regs = crate::compiler::regalloc::RegAlloc::new_with_base(
                        target.ct_next_local_reg(),
                    )
                    .expect("out of register zones");
                    let mul_reg = crate::compiler::expr::compile_reg::compile_expr_to_reg(
                        target, mul_expr, &mut regs,
                    );
                    let add_reg = crate::compiler::expr::compile_reg::compile_expr_to_reg(
                        target, add_expr, &mut regs,
                    );
                    target.ct_emit(Instruction::NumMulAddAssign {
                        dst: local_reg,
                        mul: mul_reg,
                        add: add_reg,
                    });
                    ...
                    target.ct_set_local_type(
                        name,
                        crate::compiler::expr::infer_type_with_locals(value, &|n: &str| {
            ...
            // B4
            if let Some(imm) =
                crate::compiler::stmt_shared::assignment_fusions::try_self_sub_int(value, name)
            {
                if let Some(local_reg) = target.ct_local_reg(name) {
                    if target.ct_is_const_local(name) {
                        return Err(compile_codes::generic(format!(
            ...
            if target.ct_local_type(name) != crate::compiler::expr::ExprType::Str {
                if let Some(imm) =
                    crate::compiler::stmt_shared::assignment_fusions::try_self_add_int(value, name)
                {
                    if let Some(local_reg) = target.ct_local_reg(name) {
                        if target.ct_is_const_local(name) {
                            return Err(compile_codes::generic(format!(
    ...
    } = assign_target
    {
        let mut regs =
            crate::compiler::regalloc::RegAlloc::new_with_base(target.ct_next_local_reg())
                .expect("out of register zones");
        let obj_reg =
            crate::compiler::expr::compile_reg::compile_expr_to_reg(target, object, &mut regs);
        let val_reg =
            crate::compiler::expr::compile_reg::compile_expr_to_reg(target, value, &mut regs);
        ...
        if let Expr::Identifier(ref arr_name, _) = &**object {
            ...
            if let Some((arr_reg, idx_reg)) = fast_regs {
                ...
                if arr_ty == crate::compiler::expr::ExprType::Array {
                    target.ct_emit(Instruction::IndexAssignArray {
                        obj: arr_reg,
                        idx: idx_reg,
                        val: val_reg,
                    });
                } else {
                    target.ct_emit(Instruction::IndexAssign {
                        obj: arr_reg,
                        idx: idx_reg,
                        val: val_reg,
                    });
            ...
            let obj_reg =
                crate::compiler::expr::compile_reg::compile_expr_to_reg(target, object, &mut regs);
            let idx_reg =
                crate::compiler::expr::compile_reg::compile_expr_to_reg(target, index, &mut regs);
            let val_reg =
                crate::compiler::expr::compile_reg::compile_expr_to_reg(target, value, &mut regs);
            target.ct_emit(Instruction::IndexAssign {
                obj: obj_reg,
