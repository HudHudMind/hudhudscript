use super::*;
use hudhudscript_bytecode::{Bytecode, Instruction, Value16, FunctionChunk};
use std::sync::Arc;
...
impl Compiler {
    ...
    pub(crate) fn compile_function_chunk_with<F>(
        &mut self, params: Vec<String>, fn_name: Option<String>,
        is_async: bool, emit_body: F,
    ) -> CompileResult<FunctionChunk>
    where F: FnOnce(&mut Self) -> CompileResult<()>
    {
        ...
        let saved_scope_depth = self.scope_depth; self.scope_depth = 0;
        let saved_declared_fns = std::mem::replace(&mut self.declared_fns, vec![HashMap::new()]);
        let saved_in_top_level = self.in_top_level; self.in_top_level = false;
        let saved_max_register = self.current_max_register; self.current_max_register = 0;
        ...
        self.fn_ctx = Some(FuncCtx { params: params.clone(), fn_name: fn_name.clone(),
            has_rest: params.last().map(|p| p.starts_with("...")).unwrap_or(false),
            referenced: Vec::new(), is_async });

        // ── Set up function-scope locals from parameters ───────────────
        let mut local_slot_names = Vec::new(); let mut locals = Vec::new();
        for (i, p) in params.iter().enumerate() {
            let slot_name = p.strip_prefix("...").map(|s| s.to_string()).unwrap_or_else(|| p.clone());
            local_slot_names.push(slot_name.clone());
            locals.push(Local { name: slot_name, depth: 0, is_captured: false,
                slot: Some(i as u32), reg: Some(i as u8), is_const: false,
                known_type: crate::compiler::expr::ExprType::Unknown });
        }
        self.locals = locals; self.local_slot_names = local_slot_names;
        ...
        if let Err(e) = &emit_result {
            ...
            self.fn_ctx = saved_fn_ctx; self.bytecode = saved_bytecode;
            self.locals = saved_locals; self.local_slot_names = saved_slot_names;
            self.scope_depth = saved_scope_depth; self.declared_fns = saved_declared_fns;
        ...
        while sp.len() < self.bytecode.instructions.len() { sp.push(None); }
        sp.truncate(self.bytecode.instructions.len());
        {
            let num_consts = std::mem::take(&mut self.bytecode.numeric_constants);
            let int_consts = std::mem::take(&mut self.bytecode.int_constants);
            crate::optimizer::fuse_slot_immediate_with_positions(
                &mut self.bytecode.instructions, &num_consts, &int_consts,
                &mut self.bytecode.loop_payloads, &mut sp);
            crate::optimizer::fuse_intmodcmpi_chain(
                &mut self.bytecode.instructions, &mut self.bytecode.loop_payloads, &mut sp);
            self.bytecode.numeric_constants = num_consts;
            self.bytecode.int_constants = int_consts;
        }
        crate::optimizer::fuse_super_instructions_with_positions(
            &mut self.bytecode.instructions, &mut self.bytecode.loop_payloads,
            &self.bytecode.call_payloads, &mut self.bytecode.super_instr_payloads, &mut sp);
        ...
        if !func_numeric.is_empty() {
            ...
            for bits in &func_numeric { index_map.push(outer_bytecode.add_numeric_constant_bits(*bits)); }
            for instr in &mut func_instructions {
                if let Instruction::LoadNumConst { const_idx, .. } = instr { *const_idx = index_map[*const_idx as usize] as u16; }
            }
                ...
                if let Some(c) = Arc::get_mut(chunk) {
                    for instr in &mut c.instructions {
                        if let Instruction::LoadNumConst { const_idx, .. } = instr {
                            if let Some(&new_idx) = index_map.get(*const_idx as usize) { *const_idx = new_idx as u16; }
                        }
                    }
                }
            }
        ...
        if !func_int.is_empty() {
            ...
            for v in &func_int { index_map.push(outer_bytecode.add_int_constant(*v)); }
            for instr in &mut func_instructions {
                if let Instruction::LoadIntConst { const_idx, .. } = instr { *const_idx = index_map[*const_idx as usize] as u16; }
            }
            for chunk in func_nested.iter_mut() {
                if let Some(c) = Arc::get_mut(chunk) {
                    for instr in &mut c.instructions {
        ...
        if !func_loop.is_empty() {
            ...
            for instr in &mut func_instructions { if let Instruction::LoopBegin(idx) = instr { *idx += base; } }
        }
        ...
        if !func_call_payloads.is_empty() {
            ...
            for sp in func_super.iter_mut() { sp.call_idx += base as u32; }
        ...
        if !func_super.is_empty() {
            ...
            for instr in &mut func_instructions { match instr {
        ...
        for (name, &idx) in func_names.iter() { if idx < name_by_idx.len() { name_by_idx[idx] = name.clone(); } }
        ...
        let mut captures = Vec::new(); let mut seen = HashSet::new();
        for name in &referenced {
            if !named_params.contains(name) && !self.top_level_names.contains(name) && !seen.contains(name) {
                seen.insert(name.clone()); captures.push(name.clone());
            }
        }
        ...
        if let Some(outer_fn_ctx) = &mut saved_fn_ctx {
            for cap in &captures { outer_fn_ctx.referenced.push(cap.clone()); }
        }

        // ── Restore outer compiler state ───────────────────────────────
        self.fn_ctx = saved_fn_ctx;
        self.bytecode = outer_bytecode; self.locals = saved_locals;
        self.local_slot_names = saved_slot_names; self.scope_depth = saved_scope_depth;
        self.declared_fns = saved_declared_fns; self.in_top_level = saved_in_top_level;
        let max_register = self.current_max_register;
        self.current_max_register = saved_max_register; self.next_local_reg = saved_next_local_reg;
        ...
        while sp.len() < func_instructions.len() { sp.push(None); }
        sp.truncate(func_instructions.len());

        // P1: plain function = no captures, non-async, non-generator
        let is_plain = !is_async && !false /* is_generator */
            && captures.is_empty() && true /* capture_cells is vec![] */;

        Ok(FunctionChunk { params: params.clone(), instructions: func_instructions,
            constants: func_constants, captures, is_async, is_generator: false,
            local_count: func_local_count, local_names: func_local_names,
            capture_cells: vec![], max_register,
            sym_to_slot: std::sync::OnceLock::new(), source_positions: sp,
