...
impl Compiler {
    pub(super) fn compile_decl_subject(
        ...
        for cap_def in ability_defs {
            ...
            self.bytecode.add_function(chunk_name, chunk_arc);
        }
    ...
    pub(super) fn compile_decl_role(
        ...
        self.known_roles
            .insert(name.to_string(), capabilities.to_vec());
        use std::collections::HashMap;
    ...
    pub(super) fn compile_decl_compose(
        ...
        compose_obj.insert(
            "base".to_string(),
            Value16::string(base_subject.to_string()),
        );
        ...
        for rule in rules {
            ...
            rule_obj.insert(
                "ability".to_string(),
                Value16::string(rule.ability_name.clone()),
            );
            match &rule.mode {
