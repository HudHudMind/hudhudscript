...
pub fn inline_small_functions(
    ...
    while i < instructions.len() {
        ...
        let has_side_effect = body.iter().any(|ci| {
            matches!(
                ci,
        ...
        for ci in body {
            ...
            if let Instruction::IntAddReturn { .. }
            | Instruction::IntSubReturn { .. }
            | Instruction::IntMulReturn { .. }
            | Instruction::IntCmpIReturn { .. } = ci
            {
...
fn remap_instr(
    ...
    let map = |r: u8| -> u8 { remap_reg(r, first_arg, arg_count, dst, base) };

    match *instr {
        Instruction::Move { dst: d, src: s } => Instruction::Move {
            dst: map(d),
            src: map(s),
        },
        Instruction::IntAdd {
        ...
        Instruction::Neg { dst: d, src: s } => Instruction::Neg {
            dst: map(d),
            src: map(s),
        },
        Instruction::Not { dst: d, src: s } => Instruction::Not {
            dst: map(d),
            src: map(s),
        },
        Instruction::LoadIntConst { dst: d, const_idx } => Instruction::LoadIntConst {
            dst: map(d),
            const_idx,
        },
        Instruction::LoadConst { dst: d, const_idx } => Instruction::LoadConst {
            dst: map(d),
            const_idx,
        },
        Instruction::LoadNumConst { dst: d, const_idx } => Instruction::LoadNumConst {
            dst: map(d),
            const_idx,
        },
