//! Messaging integrations — Telegram bot API and generic webhook POST.

use std::collections::HashMap;
use hudhudscript_bytecode::Value16;
use hudhudscript_errors::HudHudResult;

use super::helpers::{runtime_error, type_error, value_to_json_string};
...
pub fn email_telegram_send(args: &[Value16]) -> HudHudResult<Value16> {
    ...
    let bot_token = args[0]
        .as_str()
        .ok_or_else(|| type_error("string", args[0].type_name_str(), "email.telegram_send bot_token"))?
        .to_string();
    let chat_id = args[1]
        .as_str()
        .ok_or_else(|| type_error("string", args[1].type_name_str(), "email.telegram_send chat_id"))?
        .to_string();
    let text = args[2]
        .as_str()
        .ok_or_else(|| type_error("string", args[2].type_name_str(), "email.telegram_send text"))?
        .to_string();
