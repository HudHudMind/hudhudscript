...
fn parse_attach_multi_step() {
    let s = parse("attach step build with gate ok, step test with gate ok to build_loop")
        .expect("parse failed");
    match &s[0] {
        Stmt::Decl(Decl::AttachStep {
            targets, loop_name, ..
        }) => {
...
fn parse_step_gate_is_captured() {
    use hudhudscript_ast::*;
    use hudhudscript_parser::parse;
    let stmts = parse("loop e { step s { let x = 0; gate g { when x==0 -> done else -> fail } } }")
        .unwrap();
    match &stmts[0] {
        Stmt::Decl(Decl::Loop { items, .. }) => {
            ...
            match &items[0] {
                LoopItemAst::InlineStep(step_decl) => match step_decl.as_ref() {
                    Decl::Step { gate, body, .. } => {
