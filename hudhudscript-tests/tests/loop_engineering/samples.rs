...
fn sample_01_simple_done_compiles() {
    ...
    assert!(
        ...
        "loop chunk must have step body instructions"
    );
}
...
fn unsupported_gate_target_loop_is_compile_error() {
    let src =
        "loop L { step s { let x = 0; gate g { when x==0 -> loop other_loop else -> done } } }";
    let stmts = hudhudscript_parser::parse(src).unwrap();
...
fn vm_execute_simple_loop() {
    ...
    assert!(
        chunk.instructions.len() > 3,
        "loop chunk with step body must have >3 instructions, got {}",
...
fn vm_execute_run_loop_with_call() {
    ...
    assert!(
        result.is_ok(),
        "VM execute with run loop should succeed: {:?}",
        result.err()
    );
}
...
fn vm_execute_run_loop_has_loop_chunk() {
    ...
    let has_set_prop = chunk
        .instructions
        .iter()
        .any(|i| matches!(i, Instruction::SetProperty { .. }));
    assert!(
        has_set_prop,
        "done gate must emit SetProperty for success field"
    );
...
fn done_loop_has_success_true_constant() {
    ...
    let has_true = chunk
        .constants
        .iter()
        .any(|v: &hudhudscript_bytecode::Value16| v.as_bool() == Some(true));
    assert!(
        has_true,
        "done loop must have bool(true) constant for success field"
    );
}
...
fn fail_loop_has_success_false_constant() {
    ...
    let has_false = chunk
        .constants
        .iter()
        .any(|v: &hudhudscript_bytecode::Value16| v.as_bool() == Some(false));
    assert!(
        has_false,
        "fail loop must have bool(false) constant for success field"
    );
}
...
fn vm_execute_done_loop_returns_result_object() {
    ...
    let has_success = chunk
        .constants
        .iter()
        .any(|v: &hudhudscript_bytecode::Value16| v.as_bool() == Some(true));
    let has_status = chunk
        .constants
        .iter()
        .any(|v: &hudhudscript_bytecode::Value16| v.as_string().map_or(false, |s| s == "done"));
...
fn vm_execute_loop_has_bytecode_with_setproperty() {
    ...
    let set_count = chunk
        .instructions
        .iter()
        .filter(|i| matches!(i, Instruction::SetProperty { .. }))
        .count();
    assert!(
        set_count >= 2,
        "done loop must have >=2 SetProperty (success + status), got {}",
        set_count
    );
}
...
fn done_loop_has_status_done_in_constants() {
    ...
    assert!(chunk
        .constants
        .iter()
        .any(|v: &hudhudscript_bytecode::Value16| v.as_string().map_or(false, |s| s == "failed")));
    let mut vm = hudhudscript_vm::VM::new();
    assert!(vm.execute(&bytecode).is_ok());
}
...
fn retry_gate_target_jumps_back() {
    let src =
        "loop L { step S { result.x = 0; gate g { when result.x==0 -> retry else -> done } } }";
    let stmts = hudhudscript_parser::parse(src).unwrap();
    let mut compiler = Compiler::default();
...
fn chain_with_two_loops_executes_both() {
    ...
    assert_eq!(
        call_count, 2,
        "chain with 2 links must have 2 Call instructions"
    );
    assert!(
        chain_chunk.instructions.len() > 4,
        "chain must have more than skeleton"
    );
}
...
fn done_loop_bytecode_contains_success_true_constant() {
    use hudhudscript_bytecode::Value16;
    let src =
        "loop L { step S { result.x = 0; gate G { when result.x==0 -> done else -> fail } } }";
    let stmts = hudhudscript_parser::parse(src).unwrap();
    let mut compiler = Compiler::default();
    let bc = compiler.compile(&stmts).unwrap();
    let chunk = bc.get_function("__loop::L").unwrap();
...
fn fail_loop_bytecode_contains_status_failed_string() {
    use hudhudscript_bytecode::Value16;
    let src =
        "loop L { step S { result.x = 0; gate G { when result.x==1 -> done else -> fail } } }";
    let stmts = hudhudscript_parser::parse(src).unwrap();
    let mut compiler = Compiler::default();
    let bc = compiler.compile(&stmts).unwrap();
    let chunk = bc.get_function("__loop::L").unwrap();
...
fn loop_with_multiple_steps_compiles_all_steps() {
    ...
    assert!(
        set_count >= 4,
        "two steps with done gate must have >=4 SetProperty, got {}",
        set_count
    );
}
...
fn loop_with_retry_gate_has_jump_instruction() {
    let src =
        "loop L { step S { result.c = 0; gate G { when result.c==0 -> retry else -> done } } }";
    let stmts = hudhudscript_parser::parse(src).unwrap();
    let mut compiler = Compiler::default();
    let bc = compiler.compile(&stmts).unwrap();
    let chunk = bc.get_function("__loop::L").unwrap();
...
fn mode_times_emits_intsubi() {
    ...
    let has_intsub = chunk.instructions.iter().any(|i| {
        matches!(
            i,
            Instruction::IntSubI { .. } | Instruction::IntSubIJump { .. }
        )
    });
    assert!(
        has_intsub,
        "times(N) loop must emit IntSubI (or fused IntSubIJump) for counter decrement"
    );
    let has_loadint = chunk
        .instructions
        .iter()
        .any(|i| matches!(i, Instruction::LoadIntConst { .. }));
    assert!(
        has_loadint,
        "times(N) must emit LoadIntConst for counter init"
    );
}
