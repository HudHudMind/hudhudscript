...
fn sample_06_times_until_modes() {
    let vm = compile_and_execute(include_str!("../../../examples/09-loop-engineering/06_times_until.hud"));
    let obj = result_obj(&vm);
...
fn sample_07_chain_fail_short_circuits() {
    let obj = result_obj(&compile_and_execute(include_str!("../../../examples/09-loop-engineering/07_chain_fail_short.hud")));
...
fn sample_08_retry_bounded_escalates() {
    let obj = result_obj(&compile_and_execute(include_str!("../../../examples/09-loop-engineering/08_retry_bounded.hud")));
    assert_eq!(required_str(&obj, "status"), String::from("escalated"));
}

#[test]
fn sample_09_step_transition_skips_s2() {
    let obj = result_obj(&compile_and_execute(include_str!("../../../examples/09-loop-engineering/09_step_transition.hud")));
...
fn sample_10_until_exact_stop() {
    let vm = compile_and_execute(include_str!("../../../examples/09-loop-engineering/10_until_exact.hud"));
    assert_eq!(required_vm_int(&vm, "counter"), 3);
    let obj = result_obj(&vm);
    assert_eq!(required_bool(&obj, "success"), true);
}
