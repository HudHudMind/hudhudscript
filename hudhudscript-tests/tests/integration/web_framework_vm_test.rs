...
#[test]
fn test_web_escape() {
    let (_vm, result) = vm_run(r#"var x = Web.escape("<script>alert('x')</script>");"#, "x");
    assert_eq!(
...
fn test_web_html_response() {
    let (_vm, result) = vm_run(r#"var x = Web.html("<h1>Hello</h1>");"#, "x");
...
fn test_web_json_response() {
    let (_vm, result) = vm_run(r#"var x = Web.json({ ok: true });"#, "x");
    let obj = result.as_object().unwrap();
    assert_eq!(
        obj.get("content_type").unwrap().as_str().unwrap(),
        "application/json"
...
fn test_web_redirect() {
    let (_vm, result) = vm_run(r#"var x = Web.redirect("/login", 301);"#, "x");
    let obj = result.as_object().unwrap();
...
fn test_web_markdown_heading() {
    let (_vm, result) = vm_run(r##"var x = Web.markdown("# Başlık");"##, "x");
