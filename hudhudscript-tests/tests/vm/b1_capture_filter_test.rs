...
#[test] fn b1_forward_ref() {
    let r = run(r#"fn main() { return helper(10); } fn helper(x) { return x * 2; } print(main());"#).unwrap();
    assert_eq!(r, "20", "forward ref, got: {}", r);
}
#[test] fn b1_mutual_recursion() {
...
#[test] fn b1_nested_fn() {
    let r = run(r#"fn outer() { fn inner(x) { return x+1; } return inner(5); } print(outer());"#).unwrap();
    assert_eq!(r, "6", "nested fn, got: {}", r);
}
#[test] fn b1_lang2_unchanged() {
    let r = run(r#"fn outer(){ let n=1; let inc=()=>{n=n+1;}; inc();inc(); return n; } print(outer());"#).unwrap();
